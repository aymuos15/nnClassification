...
def test_tta_transform_brightness_contrast():
    """Test TTA brightness and contrast augmentations produce additional variants."""
    tta = TTATransform(augmentations=["brightness", "contrast"])
    image = torch.randn(3, 32, 32)

    augmented = tta.apply(image)

    # Original + 2 brightness variants + 2 contrast variants = 5 images
    assert len(augmented) == 5


def test_tta_apply_batch_matches_expected_shapes():
    """Ensure apply_batch returns batches matching input shape."""
    tta = TTATransform(augmentations=["horizontal_flip", "rotate_90"])
    batch = torch.randn(4, 3, 32, 32)

    augmented_batches = tta.apply_batch(batch)

    # Original + horizontal flip + rotation = 3 batches
    assert len(augmented_batches) == 3
