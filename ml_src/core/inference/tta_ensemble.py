    ...
    def run_inference(self, model, dataloader, dataset_size, device, class_names=None):
        ...
        for batch_idx, (inputs, labels) in enumerate(dataloader):
            ...
            for i in range(batch_size):
                ...
                augmented_images = self.tta_transform.apply(image)
                aug_batch = torch.stack(augmented_images).to(device)

                # Collect predictions from all models on all augmentations
                model_predictions = []

                with torch.no_grad():
                    for model_instance in self.models:
                        # Run model on all augmented versions
                        outputs = model_instance(aug_batch)

                        # Aggregate TTA predictions for this model
                        tta_aggregated = aggregate_predictions(
                            [outputs[j : j + 1] for j in range(outputs.shape[0])],
                            method=self.tta_aggregation,
                        )

                        model_predictions.append(tta_aggregated)

                # Aggregate ensemble predictions
                ensemble_aggregated = self._aggregate_ensemble_logits(model_predictions)
                batch_predictions.append(ensemble_aggregated)
