...
from ml_src.checkpointing import (
    save_checkpoint,
    save_summary,
    count_parameters
)
...
def train_model(model, criterion, optimizer, scheduler, dataloaders, dataset_sizes, device, config, run_dir, class_names,
                start_epoch=0, resume_best_acc=0.0, resume_train_losses=None, resume_val_losses=None,
                resume_train_accs=None, resume_val_accs=None):
    """
    Train the model.
    ...
    if start_epoch == 0:
        save_checkpoint(
            model, optimizer, scheduler, 0, best_acc,
