...
│   └── classN/
│
└── splits/                 # Generated by ml-split (DO NOT create manually)
...
- `splits/` - Contains index files that reference images in `raw/` (generated by `ml-split`)
...
4. ✅ **splits/ directory:** Generated automatically by `ml-split`
...
Use `ml-split` to generate index files for k-fold cross-validation:
...
**Parameters:**
- `--raw_data`: Path to your `raw/` directory (required)
- `--output`: Where to save split index files (optional, auto-derived from raw_data if not specified)
...
**Examples:**
```bash
# Simplest usage (output auto-derived)
ml-split --raw_data data/my_dataset/raw

# With custom folds and ratio
ml-split --raw_data data/my_dataset/raw --folds 10 --ratio 0.8 0.1 0.1

# With custom output location
ml-split --raw_data data/my_dataset/raw --output custom/location
```
...
**Important:** The test set is held out ONCE at the beginning and is the SAME for all folds. Only train/val splits vary across folds. This ensures fair cross-validation evaluation.
...
│       └── ... (250 images total)
└── splits/
    ├── test.txt (15% of images, SAME for all folds)
    ├── fold_0_train.txt (70% of remaining images)
    ├── fold_0_val.txt (15% of remaining images)
    ├── fold_1_train.txt
    ├── fold_1_val.txt
    └── ...
```
...
ml-split --raw_data data/hymenoptera_data/raw --folds 5
# Output: data/hymenoptera_data/splits/

# Train different folds
ml-train --config configs/hymenoptera_config.yaml --fold 0  # Creates runs/hymenoptera_base_fold_0/
ml-train --config configs/hymenoptera_config.yaml --fold 1  # Creates runs/hymenoptera_base_fold_1/
ml-train --config configs/hymenoptera_config.yaml --fold 2  # Creates runs/hymenoptera_base_fold_2/
```
...
if [ ! -d "data/my_dataset/splits" ]; then
  ...
  echo "   Run: ml-split --raw_data data/my_dataset/raw"
  exit 1
fi
...
# Check split sizes
echo "Split sizes:"
echo "  Test (shared): $(wc -l < data/my_dataset/splits/test.txt) images"
echo ""
echo "Fold 0:"
...
- **Val:** Used for hyperparameter tuning and model selection (varies across folds)
- **Test:** Final evaluation (held out ONCE, SAME for all folds)

**Important Cross-Validation Design:**
- ✅ Test set is held out ONCE at the beginning (15% of all data)
- ✅ Remaining 85% is split into train/val across k folds
- ✅ Each fold has different train/val split, but SAME test set
- ✅ This ensures test set is truly unseen and fair for all folds

**Why not different test sets per fold?**
Using different test sets per fold (with different random seeds) would give each fold different evaluation data, making results incomparable. Our approach ensures all folds are evaluated on the exact same held-out test set.
...
FileNotFoundError: Index file not found: data/my_dataset/splits/test.txt
```
...
- 1 test.txt file (shared across all folds)
- N×2 train/val files (fold_0_train.txt, fold_0_val.txt, fold_1_train.txt, etc.)
...
print("ml-split --raw_data data/my_dataset/raw")
```
...
4. **Document your splits** - Note the exact `ml-split` command used
...
- [ ] Generated splits using `ml-split`
