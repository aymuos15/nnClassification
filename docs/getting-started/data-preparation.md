...
6. **Version control** your organization script or index files
7. **Backup original data** before reorganizing

### CV-Specific Best Practices

1. **Commit index files to git** - They're small and ensure reproducibility
2. **Use consistent seeds** - Same seed = same splits
3. **Keep raw data immutable** - Never modify files in `raw/`
4. **Document split generation** - Save the exact command used
5. **Test fold 0 first** - Verify workflow before running all folds
