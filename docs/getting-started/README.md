...
```bash
# Update ml_src/config_template.yaml with your dataset info
# Then train
ml-train --fold 0
```
...
**Base configuration:** `ml_src/config_template.yaml`
```yaml
data:
...
| `ml-inference` | Run evaluation on test set | `ml-inference --checkpoint_path runs/base/weights/best.pt` |
...
# Evaluate all folds
for fold in {0..4}; do
  ml-inference --checkpoint_path runs/my_dataset_base_fold_$fold/weights/best.pt
done
```
